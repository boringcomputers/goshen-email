...
test('setup follows mailbox selection before message loading and after failures', { timeout: 60_000 }, async (t) => {
  ...
  await page.goto(base + '/app#/inboxes/' + encodeURIComponent(a))
  ...
  await page.goto(base + '/app#/inboxes/' + encodeURIComponent(b))
  await page.locator('#get-started').click()
  await requested
  ...
  await page.goto(base + '/app#/inboxes/' + encodeURIComponent(c))
  ...
  await page.waitForFunction(() => document.querySelector('#inbox-count').textContent === '2 inboxes')
  await page.locator('#get-started').click()
  await page.locator('#setup-address').filter({ hasText: a }).waitFor()
