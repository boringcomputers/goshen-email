---
title: Inboxes
description: An inbox is a real email address with its own message store. Create one per agent, per project, or per task.
---
...
`username` is required. It becomes the local part of the address on the default domain, `{{DEFAULT_DOMAIN}}`. Usernames are 1 to 64 characters, start with a letter or digit, and may contain letters, digits, dots, underscores, and hyphens.
