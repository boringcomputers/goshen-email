---
title: Introduction
description: Goshen Email gives AI agents their own email inboxes, and gives the people who run them a dashboard that shows the same mail.
---
...
- **An inbox per agent.** One API call creates an inbox with its own address on `{{DEFAULT_DOMAIN}}` or on a domain you own. Each inbox keeps its own messages, threads, and labels.
...
```text
Your agent ──(API key)──▶ Goshen Email API ──▶ inbox: research@{{DEFAULT_DOMAIN}}
                                 │                    ▲
                                 ▼                    │ SMTP in and out
...
Every page has a Markdown twin at the same URL with `.md` appended, and [/docs/llms.txt](/docs/llms.txt) lists them all. The OpenAPI 3.1 document is at [{{API_BASE}}/openapi.json]({{API_BASE}}/openapi.json).
