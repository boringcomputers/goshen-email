---
title: CLI
description: A JSON command-line client for scripts and shells. Every API operation, dry runs, and schema output.
---

## Availability

The CLI is `bezalel-email`, built from the Goshen Email repository and not yet published to npm. From a checkout after `pnpm build`, run it as `node packages/email-cli/dist/main.js`. Node.js 24 is required. The examples below assume the binary is on your `PATH`.

## Configuration

```sh
export BEZALEL_API_KEY="bze_..."
export BEZALEL_BASE_URL="{{API_BASE}}"   # optional; defaults to the hosted API
```
...
```text
bezalel-email <resource> <command> [--json <JSON|->] [flags]
```
...
```sh
bezalel-email inboxes create --username research --group agents
bezalel-email messages list --inbox-id research@{{DEFAULT_DOMAIN}} --labels received --labels unread --limit 20
bezalel-email messages send --inbox-id research@{{DEFAULT_DOMAIN}} \
...
```sh
bezalel-email inboxes update --json '{"inboxId":"research@{{DEFAULT_DOMAIN}}","group":null}'
bezalel-email messages send --json - < approved-message.json
```
...
```sh
bezalel-email threads list --inbox-id research@{{DEFAULT_DOMAIN}} --labels unread | jq -r '.threads[].subject'
```
...
```sh
bezalel-email messages send --dry-run --inbox-id research@{{DEFAULT_DOMAIN}} --to a@example.net --text hi --idempotency-key k1
```

`--schema` after a command prints that operation's JSON Schema; `bezalel-email --schema` prints all of them. Useful for generating forms or validating input in another language.
...
| Flag | Meaning |
| --- | --- |
| `--base-url <origin>` | Override `BEZALEL_BASE_URL` |
