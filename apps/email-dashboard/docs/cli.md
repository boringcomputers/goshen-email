...
export GOSHENEMAIL_BASE_URL="{{API_BASE}}"   # optional; defaults to the hosted API
```
...
goshenemail messages list --inbox-id research@{{DEFAULT_DOMAIN}} --labels received --labels unread --limit 20
goshenemail messages send --inbox-id research@{{DEFAULT_DOMAIN}} \
...
```sh
goshenemail inboxes update --json '{"inboxId":"research@{{DEFAULT_DOMAIN}}","group":null}'
...
```sh
goshenemail threads list --inbox-id research@{{DEFAULT_DOMAIN}} --labels unread | jq -r '.threads[].subject'
```
...
```sh
goshenemail messages send --dry-run --inbox-id research@{{DEFAULT_DOMAIN}} --to a@example.net --text hi --idempotency-key k1
```
