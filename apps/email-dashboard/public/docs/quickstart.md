...
```sh
curl "$BEZALEL_BASE_URL/v1/inboxes/research%40agents.goshenemail.com/messages/send" \
  -H "Authorization: Bearer $BEZALEL_API_KEY" \
  -H "Content-Type: application/json" \
  -d '{
...
```sh
curl "$BEZALEL_BASE_URL/v1/inboxes/research%40agents.goshenemail.com/messages?labels=received&labels=unread" \
  -H "Authorization: Bearer $BEZALEL_API_KEY"
```
...
```sh
curl "$BEZALEL_BASE_URL/v1/inboxes/research%40agents.goshenemail.com/threads/0b8d0e7f-3444-4bb7-a250-c2793dd5944d?includeBodies=true" \
  -H "Authorization: Bearer $BEZALEL_API_KEY"
```
...
```sh
curl "$BEZALEL_BASE_URL/v1/inboxes/research%40agents.goshenemail.com/messages/<message-id>/reply" \
  -H "Authorization: Bearer $BEZALEL_API_KEY" \
  -H "Content-Type: application/json" \
