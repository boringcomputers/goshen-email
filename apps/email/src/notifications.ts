...
export async function desktopNotifications(db: Database, customerId: string) {
  const notifications = await db.query<{ id: string; inboxId: string; createdAt: string }>(
    `select n.id, i.address as "inboxId", n.created_at as "createdAt" from mail.notifications n ${eligible}
     ...
     order by n.created_at desc, n.id desc limit 100`, [customerId])
...
export async function sendNotifications(db: Database, transport: Transport, from: string, publicUrl: string) {
  ...
  for (let batch = 0; batch < 10; batch++) {
    ...
    const [recipient] = await db.query<{ email: string; count: number }>(
    ...
    try {
      const result = await transport.send({
        ...
        subject: "New mail in Bezalel Email",
        text: `${recipient.count} new ${recipient.count === 1 ? 'message has' : 'messages have'} arrived in your inboxes.\n\nOpen your inboxes: ${new URL('/app#/inboxes', publicUrl).href}\n\nChange notification preferences: ${new URL('/app#/settings', publicUrl).href}`,
      })
      if ([...result.delivered, ...result.queued].includes(recipient.email)) state = 'accepted'
    } catch { /* Keep failures terminal: retrying may send the same notification twice. */ }
    await db.query(`update mail.notifications set email_state = $2 where id = any($1::uuid[])`, [ids, state])
  }
}
