 ...
 * account's plan and remaining balances from Autumn.
 */
export async function accountUsage(service: MailService, store: CustomerStore, customer: Customer): Promise<UsageView> {
  ...
  if (!metering) return { billing: "disabled", plan: null, inboxes, features: [] }
  const billable: BillingCustomer = { id: customer.id, email: customer.email, ...(customer.displayName ? { name: customer.displayName } : {}) }
  if (metering.exempt(billable)) return { billing: "exempt", plan: null, inboxes, features: [] }
  ...
  return { billing: "metered", plan, inboxes, features: usage.balances }
}
