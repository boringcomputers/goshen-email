...
2. Deploy the email Worker, then the dashboard. No SMTP or DNS changes are
   ...
   If the dashboard is deployed first, the older Worker answers settings
   requests with a 404 and the Settings page reports that the email API needs
   its latest deployment, with a retry. Other pages keep working.
